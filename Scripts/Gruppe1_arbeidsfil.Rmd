...
joined_df <- og_data %>% 
  ...
  distinct() %>% # Deciding to join the original dataframe to the ad_df by the column "id" as this is present in both datasets and finally removing duplicates
...
joined_df %>% summarise(across(everything(), ~sum(is.na(.)))) #Checking for missing values in the dataset. There is 2 missing values for BMI,  1 for sore_throat, mallampati and mallampati_asa

#Stratifying the dataset for a categorical column (choosing the column gender) and reporting min, max, mean and sd of a numeric column (choosing the column total_intubation_time)
joined_df %>% 
  group_by(gender) %>% 
  summarise(
    min_total_intubation_time = min(total_intubation_time, na.rm = TRUE),
    max_total_intubation_time = max(total_intubation_time, na.rm = TRUE),
    mean_total_intubation_time = mean(total_intubation_time, na.rm = TRUE),
    sd_total_intubation_time = sd(total_intubation_time, na.rm = TRUE))


#Stratifying our data by a categorical column and reporting min, max, mean and sd of a numeric column for a defined set of observations 
