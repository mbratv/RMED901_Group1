...
# We see that id is positively correlated with randomization and last_method as well as failures with attempts and intubation time, and attempts and intubation time. Intubation_overall_S_F is negatively correlated with bleed, ease, sore throat, total_intubation_time, failures, attempts. View and ease are also negatively correlated.
...
panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
  scale_x_continuous(name = "Age",limits = c(20,80))   # Density plot of the age distribution depending on gender. We removed the grey background and the major and minor grids. We set the colour of the axis to black. We set the x-axis limit from 20 to 80.
...
# Checking if age and BMI have a linear relationship by adding geom_smooth (adds a regression line with standard deviation) to a scatterplot. Also removed the grey background and the grids. We set the x-asis limit from 20 to 80. 
```
...
The second question is answered in heat map above. The randomization is slightly negativeliy correlated with BMI. 

#Comparing Total Intubation time by Gender
```{r}
# Making a box plot to compare the two gender categories.
