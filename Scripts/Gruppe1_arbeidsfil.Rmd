...
```{r}
data <- tidy_data %>% select(-c(month, year)) # Removing the columns month and year 

ad_df <- read_delim(here("Data", "exam_data_join.txt")) # Importing the additional dataset and assigning it to an object


joined_df <- data %>% left_join(ad_df, by = "id") # Deciding to join the original dataframe to the ad_df by the column "id" as this is present in both datasets.

joined_df %>% mutate(gender)
