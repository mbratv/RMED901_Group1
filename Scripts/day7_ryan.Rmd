...
#library(GGally)
joined_df%>%
  suppressWarnings(ggcorr(label = F, label_size = 3, label_round = 2,
            label_alpha = TRUE, hjust = 1, size = 2)#moves labels to the side
+ labs(title="Correlation Map of Variables")+theme(plot.title = element_text(hjust =0.5 ))) #ads a title centered in the middle
...
joined_df %>% 
  ggplot(aes(x = age, y = BMI)) + geom_point() + geom_smooth(method = "lm", se = F) # Checking if age and BMI have a linear relationship by adding geom_smooth (adds a regression line) to a scatterplot.

joined_df %>% ggplot(aes(x = age, fill = gender)) + geom_density(alpha = 0.5)   # Density plot of the age distribution depending on gender I LIKE THIS THE BEST FOR AGE AND GENDER. also recommend writing the plots like this as opposed to the other way of writing it (like below)
...
```{r}
#checking Does number of failures depend on age?
joined_df %>% 
  ggplot(aes(x = age, y = Failures)) + geom_point() + geom_smooth(method = "lm", se = F) # does not look good at all, unless find a way to exclude 0 values
joined_df %>% ggplot(aes(x = age, fill = Failures)) + geom_density(alpha = 0.5,fill= "blue")+ #Made the space underneath the plot blue
labs(title = "Age vs Failures")+ylab("Density of Failures")+theme(plot.title = element_text(hjust =0.5 ))+scale_x_continuous(name = "Age",limits = c(20,80))+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"))
#Generally removing the background color and grid to make the plot easier to look at. The axes were renamed and the limits of the x axis were set to 20-80 in order to make the plot cleaner. Axis line colors were set to black for increased contrast.
#ggsave(here("processed_data","Age_v_Failure_density"))
#ok looks a little bit like something
ggplot(joined_df, aes(x = age, fill = Failures)) +
  geom_histogram(binwidth = 2, position = "stack")+theme_minimal()+ labs(title ="Age vs Intubation Failures")+ xlab("Age") + ylab("Number of Failures")
...
first one isn't because gender isn't on there

```{r}
joined_df%>%
  ggplot(aes(x=Randomization, fill=gender))+
  geom_histogram()
#well that doesn't look good, would like to remove values between and maybe separate male and female?
```
